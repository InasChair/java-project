...
public class EmpolyeeController implements Initializable {
    ...
    private void Add(ActionEvent event) throws SQLException {
         if(Name.getText().trim().isEmpty()==true || Password.getText().trim().isEmpty()==true || cpass.getText().trim().isEmpty()==true )
        {
              Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Warning");
            alert.setHeaderText(null);
             alert.setContentText("fill all the blancks plz!");
            alert.showAndWait();
            return;
        }
    ...
    private void change(ActionEvent event) throws SQLException {
        if(Name1.getText().trim().isEmpty()==true || Password1.getText().trim().isEmpty()==true || cpass1.getText().trim().isEmpty()==true )
        {
              Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Warning");
            alert.setHeaderText(null);
